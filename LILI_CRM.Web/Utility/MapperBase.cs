    ...
    public class MapperBase
    {
        ...
        public static T MapItem<T>(T obj, DataRow drContent)
        {
            ...
            foreach (PropertyInfo property in properties)
            {
                ...
                object[] attributes = property.GetCustomAttributes(typeof(LILI_CRM.Web.Utility.TableDataColumn), true);

                if (attributes.GetLength(0) > 0)
                {

                    if (((LILI_CRM.Web.Utility.TableDataColumn)attributes[0]).IsDataColumn())
                    {
