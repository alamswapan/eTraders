...
$(document).ready(function () {
    ...
    $("#ExpenseBudgetKendoGrid").kendoGrid({
        dataSource: {
            transport: {
                read: "/PCV/ExpenseBudget/ExpenseBudgetRead"
            },
            schema: {
                model: {
                    fields: {
        ...
        columns: [{ field: "Id", title: "Id", hidden: false, filterable: false, sortable: false, width: "10%" },
                { field: "BudgetYear", title: "BudgetYear", width: "15%" },
                { field: "SalesPersonName", title: "Sales Person", width: "25%" },
                { field: "DivisionName", title: "Division", width: "25%" },
                { field: "SBUName", title: "SBU", width: "25%" },                
    ...
    //add SampleRequest
    $('#lnkAddExpenseBudget').live('click', function () {

        //change the title of the dialog
        var linkObj = $(this);
        var dialogDiv = $('#addExpenseBudgetDialog');
    ...
    //edit SampleRequest
    $("#editExpenseBudgetDialog").dialog({
        autoOpen: false,
        width: 1250,
        resizable: false,
        modal: true
    ...
    $('#ExpenseBudgetKendoGrid tbody tr td a.lnkEditExpenseBudget').live('click', function () {

        //change the title of the dialog
        var linkObj = $(this);
        var dialogDiv = $('#editExpenseBudgetDialog');
        var viewUrl = linkObj.attr('href');
        $.get(viewUrl, function (data) {
            dialogDiv.html(data);

            //get form
            var $form = $("#editExpenseBudgetForm");
